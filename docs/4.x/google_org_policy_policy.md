...
`google-beta.list[obj].withDryRunSpec` constructs a mixin object that can be merged into the `list[obj]`
...
`google-beta.list[obj].withDryRunSpecMixin` constructs a mixin object that can be merged into the `list[obj]`
Terraform resource block to set or update the dry_run_spec field.

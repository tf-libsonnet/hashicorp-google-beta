...
lowercase letters or the hyphen character.
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google-beta.google_service_directory_namespace.timeouts.new](#fn-google_service_directory_namespacetimeoutsnew) constructor.
...
Unlike [google-beta.google_service_directory_namespace.new](#fn-google_service_directory_namespacenew), this function will not inject the `resource`
...
`google-beta.string.withNamespaceId` constructs a mixin object that can be merged into the `string`

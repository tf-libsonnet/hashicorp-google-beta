...
If not specified, a new Service will automatically be created in the same project as this API. When `null`, the `managed_service` field will be omitted from the resulting object.
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google-beta.google_api_gateway_api.timeouts.new](#fn-google_api_gateway_apitimeoutsnew) constructor.
...
Unlike [google-beta.google_api_gateway_api.new](#fn-google_api_gateway_apinew), this function will not inject the `resource`

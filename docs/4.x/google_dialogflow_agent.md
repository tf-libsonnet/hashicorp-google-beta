...
`google-beta.string.withApiVersion` constructs a mixin object that can be merged into the `string`
...
`google-beta.number.withClassificationThreshold` constructs a mixin object that can be merged into the `number`
...
`google-beta.string.withMatchMode` constructs a mixin object that can be merged into the `string`

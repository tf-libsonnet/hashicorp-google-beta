...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google-beta.google_container_azure_client.timeouts.new](#fn-google_container_azure_clienttimeoutsnew) constructor.
...
Unlike [google-beta.google_container_azure_client.new](#fn-google_container_azure_clientnew), this function will not inject the `resource`
...
`google-beta.string.withApplicationId` constructs a mixin object that can be merged into the `string`
...
`google-beta.string.withTenantId` constructs a mixin object that can be merged into the `string`

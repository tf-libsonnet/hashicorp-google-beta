...
`google-beta.string.withActiveKeyVersion` constructs a mixin object that can be merged into the `string`
...
`google-beta.list[obj].withEnrolledServices` constructs a mixin object that can be merged into the `list[obj]`
...
`google-beta.list[obj].withEnrolledServicesMixin` constructs a mixin object that can be merged into the `list[obj]`
Terraform resource block to set or update the enrolled_services field.
...
`google-beta.list.withNotificationEmails` constructs a mixin object that can be merged into the `list`

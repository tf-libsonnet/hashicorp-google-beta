...
The short name must be 63 characters or less, beginning and ending with an alphanumeric character ([a-z0-9A-Z]) with dashes (-), underscores (_), dots (.), and alphanumerics between.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google-beta.google_tags_tag_value.timeouts.new](#fn-google_tags_tag_valuetimeoutsnew) constructor.
...
Unlike [google-beta.google_tags_tag_value.new](#fn-google_tags_tag_valuenew), this function will not inject the `resource`
